namespace DESEncodeDecodeLib.AlgorithmTables
{
    ...
    static class SBoxMapper
    {
        public static SBox[] S { get; } =
        {
            new SBox // S1
            {
    ...
    class SBox : IEnumerable<byte[]>
    {
        public LinkedList<byte[]> Matrix { get; set; }

        public byte[] this[int index] => Matrix.ElementAt(index);

        #region CONSTRUCTORS

        public SBox() => Matrix = new LinkedList<byte[]>();

        #endregion

        public IEnumerator<byte[]> GetEnumerator()
        {
            foreach (byte[] byteArray in Matrix)
            {
